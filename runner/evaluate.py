...
class Evaluator(BaseEvaluator):
    ...
    def convert_batch_to_inputs(self, batch):
        # batch tuple 结构（共 25 个元素，与 collate_fn 返回顺序一致）：
        # [0]  enc_input_ids
        # [1]  enc_mask_ids
        # [2]  all_ids
        # [3]  all_mask_ids
        ...
        # [17] end_position_ids
        # [18] event_trigger          ← 原 batch[-2]，固定用 [18]
        # [19] enc_attention_mask     ← 原 batch[-1]，固定用 [19]
        # [20] dep_heads_batch        ← 新增
        # [21] dep_rels_batch         ← 新增
        # [22] event_groups_batch     ← 新增
        # [23] coref_clusters_batch   ← 共指簇
        # [24] coref_logits_batch     ← 共指置信度

        if self.cfg.model_type == "DyGMA":
            inputs = {
                ...
                'arg_list':                  batch[11],
                'event_triggers':            batch[18],   # 固定索引
                'enc_attention_mask':        batch[19],   # 固定索引
                # ── 新增三个图字段 ──────────────────────────────────────
        ...
        named_v = {
            "arg_roles":   batch[11],
            "feature_ids": batch[13],   # feature_idx，索引不变
        }
        ...
        for i in range(bs):
            ...
            for j, (prediction, roles) in enumerate(zip(predictions, list_roles)):
                for arg_role in roles:
                    [start_logits_list, end_logits_list] = prediction[arg_role]
    ...
    def predict(self):
        ...
        for (pred, feature_id, event_index, role) in zip(
            pred_list,
            self.record["feature_id_list"],
            self.record["event_index_list"],
            self.record["role_list"],
        ):
        ...
        for metric, eval_fn in self.metric_fn_dict.items():
            ...
            logger.info('{}-Classification. {} ({}): R {} P {} F {}'.format(
                metric, self.set_type, perf_c['gt_num'],
                perf_c['recall'], perf_c['precision'], perf_c['f1']))
            logger.info('{}-Identification. {} ({}): R {} P {} F {}'.format(
                metric, self.set_type, perf_i['gt_num'],
                perf_i['recall'], perf_i['precision'], perf_i['f1']))
...
#     def convert_batch_to_inputs(self, batch):
#         # batch tuple 结构（共 23 个元素，与 collate_fn 返回顺序一致）：
#         # [0]  enc_input_ids
#         # [1]  enc_mask_ids
#         # [2]  all_ids
#         # [3]  all_mask_ids
